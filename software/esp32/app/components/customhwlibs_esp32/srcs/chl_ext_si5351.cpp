...
int chl_ext_si5351::set_frequency(bool pllB, unsigned int target_freq) {
    ...
    _i2c_drv->i2c_write_regs(_addr, SI5351_REG_OUTPUT_CTRL_BASE, tmp, 3, true, true); if(_i2c_drv->i2c_wait_for_transaction() != 0) {printf("si5351 set_frequency: i2c error 0\n");return -1;} //configure CLK0,1,2
    tmp[0] = 0x00; tmp[1] = 0x00; tmp[2] = 0x00;
    _i2c_drv->i2c_write_regs(_addr, (SI5351_REG_PHASE_OFFSET_BASE), tmp, 3, true, true); if(_i2c_drv->i2c_wait_for_transaction() != 0) {printf("si5351 set_frequency: i2c error 1\n");return -1;} //0 phase offset for outs 0, 1,2
    tmp[0] = ((uint8_t)roundf((1.0/freq) / 4.0f * 4.0f * pll_freq)) & 0x7F; 
    _i2c_drv->i2c_write_regs(_addr, (SI5351_REG_PHASE_OFFSET_BASE+1), tmp, 1, true, true); if(_i2c_drv->i2c_wait_for_transaction() != 0) {printf("si5351 set_frequency: i2c error 2\n");return -1;} //+pi/2 phase difference on CLK1
    tmp[0] = pllB ? 0x80 : 0x20;
    _i2c_drv->i2c_write_regs(_addr, SI5351_REG_PLL_RESET, tmp, 1, true, true); if(_i2c_drv->i2c_wait_for_transaction() != 0) {printf("si5351 set_frequency: i2c error 3\n");return -1;} //reset PLL
...
bool chl_ext_si5351::reset_pll(int pll) {
    ...
    _i2c_drv->i2c_write_regs(_addr, SI5351_REG_PLL_RESET, tmp, 1, true, true); if(_i2c_drv->i2c_wait_for_transaction() != 0) {printf("si5351 reset_pll: i2c error\n");return false;} //reset PLL
    return true;
}
...
bool chl_ext_si5351::set_pll_frequency_shift(bool pllB, int32_t target_shift_hz) {
    ...
    if(abs(bdiff) > 499998) {
        bdiff = (bdiff > 0) ? 499999 : -499999;
    }
    uint32_t newb = 500000 + bdiff;
    if(!_sync_set_divider((pllB ? 2 : 1), (pllB ? SI5351_REG_PLLB_MUL_BASE : SI5351_REG_PLLA_MUL_BASE), (pllB ? _prev_pllb_a : _prev_plla_a), newb)) { return false;} //set pll coeffs
    return true;
}
...
bool chl_ext_si5351::set_output_enabled(bool tx, bool enabled) {
    ...
    _i2c_drv->i2c_write_regs(_addr, SI5351_REG_OUTPUT_DIS, &tmp, 1, true, true); if(_i2c_drv->i2c_wait_for_transaction() != 0) {return false;}
    return true;
}
...
void chl_ext_si5351::reset_chip() {
    ...
    tmp[0] = 0xff;
    _i2c_drv->i2c_write_regs(_addr, SI5351_REG_OUTPUT_DIS, tmp, 1, true, true); if((err = _i2c_drv->i2c_wait_for_transaction()) != 0) {printf("si5351 reset_chip: i2c error 0 %d\n", err);return;} //Disable all outputs
    tmp[0] = 0x80;
    for(uint8_t reg_addr = SI5351_REG_OUTPUT_CTRL_BASE; reg_addr < (SI5351_REG_OUTPUT_CTRL_BASE+8); reg_addr++) {
        _i2c_drv->i2c_write_regs(_addr, reg_addr, tmp, 1, true, true); if((err = _i2c_drv->i2c_wait_for_transaction()) != 0) {printf("si5351 reset_chip: i2c error 1 %d\n", err);return;} //write 0x80 to regs 16-23(powerdown all)
    }
    tmp[0] = 0x00;
    _i2c_drv->i2c_write_regs(_addr, SI5351_REG_OUTPUT_DIS_STATE, tmp, 1, true, true); if((err = _i2c_drv->i2c_wait_for_transaction()) != 0) {printf("si5351 reset_chip: i2c error 2 %d\n", err);return;} //configure CLKs disable state
    tmp[0] = 0x0F; tmp[1] = 0x0F; tmp[2] = 0x0F;
    _i2c_drv->i2c_write_regs(_addr, SI5351_REG_OUTPUT_CTRL_BASE, tmp, SI5351_REG_OUTPUT_DIS, true, true); if((err = _i2c_drv->i2c_wait_for_transaction()) != 0) {printf("si5351 reset_chip: i2c error 3 %d\n", err);return;} //configure CLK0,1,2(PLLA)
    tmp[0] = 0x00;
    _i2c_drv->i2c_write_regs(_addr, SI5351_REG_PLL_SOURCE, tmp, 1, true, true); if((err = _i2c_drv->i2c_wait_for_transaction()) != 0) {printf("si5351 reset_chip: i2c error 6 %d\n", err);return;} //Select XTAL source for both PLLs
    tmp[0] = 0x52;
    _i2c_drv->i2c_write_regs(_addr, SI5351_REG_XTAL_LOAD, tmp, 1, true, true); if((err = _i2c_drv->i2c_wait_for_transaction()) != 0) {printf("si5351 reset_chip: i2c error 7 %d\n", err);return;} //Set 6pf(min) crystal load
...
void chl_ext_si5351::set_outputs_pll(bool pllB) {
    if(!initialized) { return;}
    uint8_t tmp[3];
    tmp[0] = pllB ? SI5351_MS_CFG_PLLB : SI5351_MS_CFG_PLLA; tmp[1] = tmp[0]; tmp[2] = tmp[0];
    _i2c_drv->i2c_write_regs_isr(_addr, SI5351_REG_OUTPUT_CTRL_BASE, tmp, 3, true);
}

void chl_ext_si5351::set_pll_frequency_fast(bool pllB, int32_t target_shift_hz) {
    if(!initialized) { return;}
    int32_t bdiff = target_shift_hz / _curr_freq_step;
    if(abs(bdiff) > 499998) {
        bdiff = (bdiff > 0) ? 499999 : -499999;
    ...
    uint32_t newb = 500000 + bdiff;
    _fast_change_pll_mul(pllB, newb);
}
...
bool chl_ext_si5351::_sync_set_divider(int pll, uint8_t base_reg, uint32_t a, uint32_t b) {
    ...
    uint32_t MS0_P1 = (128 * a) + floorf((b*128)/SI5351_FRACTIONAL_DIVIER) - 512;
    uint32_t MS0_P2 = (128 * b) - (SI5351_FRACTIONAL_DIVIER * floorf((b*128)/SI5351_FRACTIONAL_DIVIER));
    ...
    _i2c_drv->i2c_write_regs(_addr, base_reg, buffer, 8, true, true); if(_i2c_drv->i2c_wait_for_transaction() != 0) {printf("si5351 _sync_set_divider: i2c error\n");return false;} //Write config for x
    return true;
}
...
void chl_ext_si5351::_fast_change_pll_mul(bool pllB, uint32_t b) {
    if(b == (pllB ? _prev_pllb_b : _prev_plla_b)) {
        //Nothing changed
        return;
    }
    uint8_t buffer[6];
    uint32_t new_pll_P1 = (128 * (pllB ? _prev_pllb_a : _prev_plla_a)) + floor((b*128)/SI5351_FRACTIONAL_DIVIER) - 512;
    uint32_t new_pll_P2 = (128 * b) - (SI5351_FRACTIONAL_DIVIER * floor((b*128)/SI5351_FRACTIONAL_DIVIER));
    ...
    if((pllB ? _prev_pllb_p1 : _prev_plla_p1) == new_pll_P1) {
        ...
        if(xPortInIsrContext()) {
            _i2c_drv->i2c_write_regs_isr(_addr, ((pllB ? SI5351_REG_PLLB_MUL_BASE : SI5351_REG_PLLA_MUL_BASE)+5), buffer, 3, true);
        } else {
            _i2c_drv->i2c_write_regs(_addr, ((pllB ? SI5351_REG_PLLB_MUL_BASE : SI5351_REG_PLLA_MUL_BASE)+5), buffer, 3, true, true);
        }
    } else {
        //changing only 6 values from (base+2) to (base+8)
        buffer[0] =  (new_pll_P1 >>     16L) & 0x03;                               //x_DIV[2:0] + x_DIVBY4[1:0] + x_P1[17:16]
        buffer[1] =  (new_pll_P1 >>      8L) & 0xff;                               //x_P1[15:8]
        buffer[2] =  (new_pll_P1           ) & 0xff;                               //x_P1[7:0]
        buffer[3] = ((pll_P3 >> 12L) & 0xf0) | ((new_pll_P2 >> 16L) & 0x0f);       //x_P3[19:16] + x_P2[19:16]
        buffer[4] =  (new_pll_P2 >>      8L) & 0xff;                               //x_P2[15:8]
        buffer[5] =  (new_pll_P2           ) & 0xff;                               //x_P2[7:0]
        if(xPortInIsrContext()) {
            _i2c_drv->i2c_write_regs_isr(_addr, ((pllB ? SI5351_REG_PLLB_MUL_BASE : SI5351_REG_PLLA_MUL_BASE)+2), buffer, 6, true);
        } else {
            _i2c_drv->i2c_write_regs(_addr, ((pllB ? SI5351_REG_PLLB_MUL_BASE : SI5351_REG_PLLA_MUL_BASE)+2), buffer, 6, true, true);
        }
    }
    (pllB ? _prev_pllb_b=b : _prev_plla_b=b);
    (pllB ? _prev_pllb_p1=new_pll_P1 : _prev_plla_p1=new_pll_P1);
}
