...
void IRAM_ATTR app_main(void) {
    ...
    dsp_mgr::n_msk_tx_set_cb(n_bfsk_tx_cb, NULL);
    dsp_mgr::n_msk_rx_set_cb(n_bfsk_rx_cb, NULL);
    packet_mgr::init();
        ...
        switch(p.type) {
            ...
            case pc_packet_interface::packetType_frompc::PC_PI_PTP_N_MFSK_TRANSMIT: {
                if(curr_mode == pc_packet_interface::modes::PC_PI_MODE_NORMAL_MFSK) {
                    if(p.len > 0) {
                        ...
                        packet_mgr::load_tx_data(p.data, p.len);
                        dsp_mgr::n_mfsk_tx_set_params(frcnt, frs, 100.0f);
            ...
            case pc_packet_interface::packetType_frompc::PC_PI_PTP_N_MFSK_START_RX: {
                if(curr_mode == pc_packet_interface::modes::PC_PI_MODE_NORMAL_MFSK) {
                    int frcnt = 8;
                    float frs[frcnt] = {-500.0f, -375.0f, -250.0f, -125.0f, 125.0f, 250.0f, 375.0f, 500.0f};
                    dsp_mgr::n_mfsk_rx_set_params(frcnt, frs, 100.0f);
            ...
            case pc_packet_interface::packetType_frompc::PC_PI_PTP_N_MSK_TRANSMIT: {
                if(curr_mode == pc_packet_interface::modes::PC_PI_MODE_NORMAL_MSK) {
                    if(p.len > 0) {
                        packet_mgr::load_tx_data(p.data, p.len);
                        dsp_mgr::n_msk_tx_set_params(200.0f);
                        dsp_mgr::n_msk_tx_start(packet_mgr::tx_reqfunc, NULL);
                        ackbyte = 1;
                    }
                }
                break;
            }
            case pc_packet_interface::packetType_frompc::PC_PI_PTP_N_MSK_START_RX: {
                if(curr_mode == pc_packet_interface::modes::PC_PI_MODE_NORMAL_MSK) {
                    dsp_mgr::n_msk_rx_set_params(200.0f);
                    dsp_mgr::n_msk_rx_start();
                    ackbyte = 1;
                }
                break;
            }
            case pc_packet_interface::packetType_frompc::PC_PI_PTP_N_MSK_STOP_RX: {
                if(curr_mode == pc_packet_interface::modes::PC_PI_MODE_NORMAL_MSK) {
